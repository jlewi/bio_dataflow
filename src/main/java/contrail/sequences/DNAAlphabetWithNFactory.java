...
public class DNAAlphabetWithNFactory {
  ...
  // unknown values?
	 ...
	 */
	private static class DNAAlphabetWithN extends Alphabet {

		/**
		 * The DNAAlphabet doesn't actually use an end of sequence character to
		 * indicate the end of the sequence; Rather we store the length of the
		 * sequence separately. For compatibility with the API we still need to
		 * define the EOS_CHAR.
		 */
		...
		public DNAAlphabetWithN() {
			...
			for (int index = 0; index < VALID_ALPHABET.length; index++){
				...
				utf8_map[ByteUtil.byteToUint(utf8_vals[0])] = letterToInt(
				    VALID_ALPHABET[index]);
			}
		 ...
		 * We do no error checking. You will get an out of bounds exception if the
	...
	public static Alphabet create() {
		if (alphabet == null){
			alphabet = new DNAAlphabetWithNFactory.DNAAlphabetWithN();
		}
		return alphabet;
	}
}